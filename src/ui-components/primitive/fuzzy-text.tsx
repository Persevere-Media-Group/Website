...
const FuzzyText: React.FC<FuzzyTextProps> = ({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // canvas's ctx.font only accepts a fully resolved size (e.g. "64px"), it can't parse
      // CSS functions like clamp()/vw. resolve those to a real pixel number FIRST, via a
      // throwaway element the browser computes for us, then build every font string used
      // below from that resolved value, not the raw fontSize prop. previously this
      // resolution happened but the result was never actually plugged back in, so any
      // clamp()/vw fontSize silently failed to parse on canvas and fell back to the
      // browser's tiny 10px default.
