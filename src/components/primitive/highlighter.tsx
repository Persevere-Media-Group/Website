...
type SvgAction = "wavy" | "zigzag";

export type HighlighterAction = RoughAction | SvgAction;

const SVG_ACTIONS = new Set<HighlighterAction>(["wavy", "zigzag"]);
...
export function Highlighter({
  ...
  if (SVG_ACTIONS.has(action)) {
    return (
      <SvgHighlighter
        action={action as SvgAction}
        color={color ?? "#ff6b9d"}
...
function SvgHighlighter({
  ...
  const highlightVariants: Variants = {
    ...
    visible: {
      ...
      opacity,
      transition: {
        pathLength: { duration: animationDuration, ease: "easeInOut", delay: animationDelay },
  ...
  const WAVY_ORIGINAL = { width: 802, height: 362 };
  const WAVY_FIXED_HEIGHT = 36;
  ...
  const ZIGZAG_ORIGINAL = { width: 729, height: 513 };
  const ZIGZAG_FIXED_HEIGHT = 40;
  ...
  const renderHighlight = () => {
    ...
    const original = action === "wavy" ? WAVY_ORIGINAL : ZIGZAG_ORIGINAL;
    const path = action === "wavy" ? WAVY_PATH : ZIGZAG_PATH;
    const svgHeight = action === "wavy" ? WAVY_FIXED_HEIGHT : ZIGZAG_FIXED_HEIGHT;
    const bottomOffsetRatio = action === "wavy" ? 0.3 : 0.2;
    ...
    return (
      <svg
        ...
        style={{
          ...svgStyle,
          bottom: `-${svgHeight * bottomOffsetRatio}px`,
          width: `${svgWidth}px`,
          height: `${svgHeight}px`,
        }}
